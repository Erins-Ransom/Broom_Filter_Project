...
static void qf_print(struct quotient_filter* qf) {
    ...
    printf(
        "| is_shifted | is_continuation | is_occupied | remainder"
        " nel=%u\n",
        qf->qf_entries);
...
static uint64_t genhash(struct quotient_filter* qf,
                        bool clrhigh,
                        set<uint64_t>& keys) {
    uint64_t hash;
...
static void supersetof(struct quotient_filter* qf,
                       struct quotient_filter* qf1,
